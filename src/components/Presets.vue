      ...
      <div class="card">
        ...
        <footer class="card-footer column-mobile">
          <div href="#" class="card-footer-item">
            <a @click="presetSelected(presets[presetAbout].software)">
              <b-icon icon="clipboard-text-play" size="is-small" />Add preset
            </a>
          </div>
        </footer>
      </div>
...
export default {
  ...
  methods: {
    presetSelected: function(preset) {
      ...
      EventBus.$emit(EventBus.installQueueChanged, [
        ...new Set([...EventBus.$installQueue, ...preset]),
      ]);
    },
